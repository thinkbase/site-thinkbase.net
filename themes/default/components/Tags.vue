<template>
  <div>
    ...
    <div class="tag">
      <div class="items">
        <span
          v-for="taginfo in tags" :key="taginfo.tag"
      ...
      <div class="article-list">
        <Article v-for="tag in infos" :key="tag.path" :tag="tag" :tg="tg" @turnTo="change" />
      </div>
    </div>
  </div>
</template>
  ...
  computed: {
    ...
    tags() {
      ...
      let all = [
        {
          tag: "全部",
          number: this.$site.pages.filter(v => v.frontmatter.tags != null && v.frontmatter.tags.length > 0).length
        }
      ];
  ...
  methods: {
    change(tag) {
      ...
      this.info = this.info.sort((pre, next) => {
        ...
        return ( //FIXME 排序的结果受 plugin "@vuepress/last-updated" 的 "transformer" 处理所影响, 参见 .vuepress/config.js
