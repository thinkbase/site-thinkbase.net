<template>
  <div class="abstract">
    <div class="abstract-item">
      ...
      <!---->
      <div class="abstract">
        <div class="tip custom-block" v-if="tag.content">
          <!-- <p class="custom-block-title">{{tag.title}}</p>
          <p>个人一些自动的 vs code 配置(Settings.json)</p>
          <ul>
            <li>VsCode 常用插件配置</li>
          </ul>-->
          <div v-html="toHtml(tag.content)"></div>
        </div>
      </div>
      ...
      <div class="article-info article-info-item">
        ...
        <i class="what" v-for="t in tag.frontmatter.tags" :key="t">
...
export default {
  ...
  methods: {
    toHtml(mdContent){
      ...
      var md = mdContent || "";  //FIXME 此处拿到的内容并不是完整的 markdown 文件, 已经被 vuepress 做过处理
      var full = true;

      var tmp = md.split("\n");
      if (tmp.length > MAX_LINES){
        tmp.length = MAX_LINES-1;
        full = false;
      }
      md = tmp.join("\n");

      if (md.length > MAX_WORDS){
        md = md.substring(0, MAX_WORDS);
        full = false;
      }
      ...
      md = md.replace(/^\s*\#/mg, "###");  //所有标题降两级

      const MarkdownIt = require('markdown-it')
      var result = (new MarkdownIt()).render(md);
      return result;
    }
  }
};
